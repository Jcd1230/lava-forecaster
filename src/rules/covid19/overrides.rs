...
fn is_supported_covid_cvx(cvx: Cvx) -> bool {
    matches!(
        cvx.0,
        cvx!("207")
            | cvx!("208")
...
fn has_in_season_dose_before_age(patient: &Patient, history: &[Dose], age: TimePeriod) -> bool {
    ...
    history.iter().any(|dose| {
        is_supported_covid_cvx(dose.cvx) && dose.date >= season_start() && dose.date < cutoff
    })
}
...
fn get_min_interval_days(
    ...
    if season == "COVID_19_AUG_2025_SEASON" {
        ...
        let age_at_season_start = compare_elapsed(
            patient.birth_date,
            season_start_dt,
            &crate::time_period!("5y"),
        );
        if season_valid_doses_len == 1 {
            if age_at_season_start == std::cmp::Ordering::Less {
                ...
                if is_novavax_or_unspec(prior_dose.cvx)
                    && is_novavax_or_unspec(current_dose_cvx)
                    && is_same_brand_or_unspec
                {
                    17
                } else {
                    52
...
pub fn evaluate_doses_seasonally(
    ...
    let mut sorted_history: Vec<Dose> = history
        .iter()
        .filter(|dose| is_supported_covid_cvx(dose.cvx))
        .cloned()
        .collect();
    ...
    for (i, dose) in sorted_history.iter().enumerate() {
        ...
        // 2. Same-day duplicate check
            ...
            if dose.cvx.0 == cvx!("313")
                && dose.date < NaiveDate::from_ymd_opt(2023, 10, 3).unwrap()
            {
                auth_ok = false;
            }
            ...
            if dose.cvx.0 == cvx!("334")
                && dose.date < NaiveDate::from_ymd_opt(2025, 8, 27).unwrap()
            {
                auth_ok = false;
            }
            ...
            if !auth_ok {
                ...
                if !is_valid_age {
                    ...
                    if is_novavax
                        && (season == "COVID_19_SEP_2023_SEASON"
                            || season == "COVID_19_AUG_2024_SEASON")
                    {
                        let season_start_dt = get_season_start_date(season);
                        let age_at_season_start = compare_elapsed(
                            patient.birth_date,
                        ...
                        let novavax_count_in_season = sorted_history
                            .iter()
                            .filter(|d| {
                                let d_season = get_covid_season(d.date);
                                d_season == season
                                    && (d.cvx.0 == cvx!("211") || d.cvx.0 == cvx!("313"))
                            })
                            .count();
                        if age_at_season_start != std::cmp::Ordering::Less
                            && novavax_count_in_season < 2
                        {
            ...
            if status == DoseStatus::Valid || status == DoseStatus::Accepted {
                ...
                if let Some(last_prior) = prior_valid_doses.last() {
                    ...
                    let season_key = if season == "COVID_19_AUG_2025_SEASON" {
                        ...
                        let is_pediatric = matches!(
                            dose.cvx.0,
                            cvx!("219") | cvx!("228") | cvx!("302") | cvx!("308") | cvx!("311")
                        );
                        if is_pediatric {
                            "PRIOR_SEASONS_LT5"
                        } else {
                    ...
                    let season_valid_doses_len = valid_doses_by_season
                        .get(season_key)
                        .map(|v| v.len())
                        .unwrap_or(0);
                    ...
                    {
                        let prior_valid_count = evaluations
                            .iter()
                            .filter(|e| {
                                e.status == DoseStatus::Valid && e.dose_date < season_start()
                            })
                            .count();
        ...
        let mut dose_number = if status == DoseStatus::Valid {
            ...
            if active_series_name == "COVID_19_AUG_2025_LT_2_SERIES"
                && season == "COVID_19_AUG_2025_SEASON"
            {
                let prior_moderna_count = sorted_history
                    .iter()
                    .filter(|d| {
                        d.date < season_start()
                            && (d.cvx.0 == cvx!("311") || d.cvx.0 == cvx!("312"))
                    })
                    .count();
                let prior_valid_count = sorted_history
                    .iter()
...
pub fn covid19_custom_evaluation_hook(
    ...
    if let Some(dose) = ctx.current_dose {
        ...
        if let Some(matching_eval) = evals
            .iter()
            .find(|e| e.dose_date == dose.date && e.cvx == dose.cvx)
        {
...
pub fn covid19_custom_forecast_hook(
    ...
    let prior_season_valid_doses: Vec<&DoseEvaluation> = evaluations
        .iter()
        .filter(|e| {
            e.status == DoseStatus::Valid
                && get_covid_season(e.dose_date) != "COVID_19_AUG_2025_SEASON"
    ...
    let last_current_season_dose = history
        .iter()
        .filter(|d| {
            is_supported_covid_cvx(d.cvx) && get_covid_season(d.date) == "COVID_19_AUG_2025_SEASON"
        })
        .max_by_key(|d| d.date);

    let last_invalid_current_season_eval = evaluations
        .iter()
        .filter(|e| {
            e.status == DoseStatus::Invalid
                && get_covid_season(e.dose_date) == "COVID_19_AUG_2025_SEASON"
    ...
    if active_series_name == "COVID_19_AUG_2025_LT_2_SERIES" {
        // Skip check
        let prior_moderna_count = history
            .iter()
            .filter(|d| {
                is_supported_covid_cvx(d.cvx)
                    && d.date < season_start()
                    && (d.cvx.0 == cvx!("311") || d.cvx.0 == cvx!("312"))
            })
            .count();
        let prior_valid_count = history
            .iter()
            .filter(|d| is_supported_covid_cvx(d.cvx) && d.date < season_start())
            .filter(|d| {
                matches!(
                    d.cvx.0,
                    cvx!("213")
        ...
        if is_skipped {
            ...
            if current_season_valid_doses.len() >= 2 {
                ...
                let overdue = crate::time_period!("8w")
                    .add_to(anchor_dose.date)
                    .pred_opt();
                ...
                // 0 current season valid doses
                forecast.mark_not_complete(crate::forecast_reasons!["NOT_COMPLETE"]);

                if let Some(invalid_eval) = last_invalid_current_season_eval {
                    let due = if invalid_eval.dose_date < age_6m {
        ...
        forecast.status = forecast.status.with_earliest_date(
            forecast
                .status
                .earliest_date()
                .map(|d| d.max(season_start()).max(age_6m)),
        );
        forecast.status = forecast.status.with_recommended_date(
            forecast
                .status
                .recommended_date()
                .map(|d| d.max(season_start()).max(age_6m)),
        ...
        forecast.status = forecast.status.with_earliest_date(
            forecast
                .status
                .earliest_date()
                .map(|d| d.max(season_start()).max(age_65y)),
        );
        forecast.status = forecast.status.with_recommended_date(
            forecast
                .status
...
pub fn covid19_group_selection(
    ...
    if let Some(forecast) = candidate_forecasts.get_forecast_mut(selected_series_name) {
        forecast.evaluations =
            evaluate_doses_seasonally(patient, history, selected_series_name).into();
        for series_forecast in forecast.forecasts.iter_mut() {
            covid19_custom_forecast_hook(patient, &[], &[], history, eval_date, series_forecast);
        }
    }
