...
macro_rules! time_period {
    ($s:expr) => {{
        const TP: $crate::date_utils::TimePeriod =
            match $crate::date_utils::TimePeriod::parse_const($s) {
                Ok(tp) => tp,
                Err(e) => panic!("{}", e),
            };
        TP
...
pub fn add_months(
    date: NaiveDate,
    duration: i32,
) -> Result<NaiveDate, crate::errors::ForecasterError> {
    if duration == 0 {
        return Ok(date);
    }
    ...
    let mut result = NaiveDate::from_ymd_opt(year, month as u32, target_day).ok_or_else(|| {
        crate::errors::ForecasterError::DateArithmeticError {
            operation: "add_months",
            detail: format!(
                "could not construct date {}-{:02}-{:02}",
    ...
    if rolled_over {
        ...
        result = result.succ_opt().ok_or_else(|| {
            crate::errors::ForecasterError::DateArithmeticError {
                operation: "add_months",
...
pub fn add_years(
    ...
    let mut result = NaiveDate::from_ymd_opt(year, month, target_day).ok_or_else(|| {
        crate::errors::ForecasterError::DateArithmeticError {
            operation: "add_years",
            detail: format!(
                "could not construct date {}-{:02}-{:02}",
...
impl TimePeriod {
    pub fn parse(s: &str) -> Result<Self, crate::errors::ForecasterError> {
        Self::parse_const(s)
            .map_err(|e| crate::errors::ForecasterError::InvalidTimePeriod(e.to_string()))
    }
    ...
    pub const fn parse_const(s: &str) -> Result<Self, &'static str> {
        ...
        while i < bytes.len() {
            ...
            parts[part_idx] = Some(TimePeriodPart {
                value: signed_value,
                unit,
            });
            part_idx += 1;
            ...
            if let Some(ref part) = self.parts[idx] {
                match part.unit {
                    DurationUnit::Days => {
                        if part.value >= 0 {
                            date = date
                                .checked_add_days(Days::new(part.value as u64))
                                .expect("date overflow in add_to (days+)");
                        } else {
                            date = date
                                .checked_sub_days(Days::new((-part.value) as u64))
                                .expect("date overflow in add_to (days-)");
                        }
                    }
                    DurationUnit::Weeks => {
                        ...
                        if total_days >= 0 {
                            date = date
                                .checked_add_days(Days::new(total_days as u64))
                                .expect("date overflow in add_to (weeks+)");
                        } else {
                            date = date
                                .checked_sub_days(Days::new((-total_days) as u64))
                                .expect("date overflow in add_to (weeks-)");
                        }
                    }
                    DurationUnit::Months => {
                        date =
                            add_months(date, part.value).expect("date overflow in add_to (months)");
                    }
                    DurationUnit::Years => {
                        date =
                            add_years(date, part.value).expect("date overflow in add_to (years)");
                    }
                }
            }
            idx += 1;
                            ...
                            date = date
                                ...
                                .ok_or_else(|| {
                                    crate::errors::ForecasterError::DateArithmeticError {
                                        operation: "add_to",
                                        detail: format!(
                                            "subtracting {} days from {} overflows",
                                            -part.value, date
                                        ),
                                    }
                                })?;
                            ...
                            date = date
                                ...
                                .ok_or_else(|| {
                                    crate::errors::ForecasterError::DateArithmeticError {
                                        operation: "add_to",
                                        detail: format!(
                                            "subtracting {} weeks from {} overflows",
                                            -part.value, date
                                        ),
                                    }
                                })?;
