...
use lava_forecaster::{evaluate_patient_all_groups, init_rayon_pool, models::UnifiedTestCase};
...
fn main() {
            ...
            && !path.to_string_lossy().contains(".expected")
        {
            ...
            match serde_json::from_str::<UnifiedTestCase>(&content) {
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "Warning: could not parse {} as UnifiedTestCase: {}",
                        path.display(),
                        e
                    );
                }
    ...
    let max_dose_case = cases
        .iter()
        .max_by_key(|tc| tc.history.len())
        .cloned()
        .expect("should have at least one case");
    println!(
        "Most complex case: {} with {} doses",
        max_dose_case.name,
        max_dose_case.history.len()
    );

    // Find a minimal case (newborn/no doses)
    let min_dose_case = cases
        .iter()
        .min_by_key(|tc| tc.history.len())
        .cloned()
        .expect("should have at least one case");
    println!(
        "Simplest case: {} with {} doses",
        min_dose_case.name,
        min_dose_case.history.len()
    );
    ...
    let iterations = 1000;
    println!(
        "Running {} iterations of all {} test cases ({} total evaluations)...",
        iterations,
        num_cases,
        iterations * num_cases
    );
    ...
    println!(
        "Throughput:         {:.2} evaluations/sec",
        evals_per_sec_st
    );
    println!(
        "Average latency:    {:.3} microseconds per patient",
        avg_latency_us_st
    );
    ...
    println!(
        "Throughput:         {:.2} evaluations/sec (Speedup: {:.2}x)",
        evals_per_sec_mt,
        evals_per_sec_mt / evals_per_sec_st
    );
    println!(
        "Average latency:    {:.3} microseconds per patient (effective)",
        avg_latency_us_mt
    );
    ...
    for _ in 0..complex_iterations {
        let res = evaluate_patient_all_groups(
            &max_dose_case.patient,
            &max_dose_case.history,
            max_dose_case.execution_date,
        );
        black_box(res);
    }
    let elapsed_st = start.elapsed();
    ...
    println!(
        "Average latency:    {:.3} microseconds (effective)",
        avg_latency_us_mt
    );
    ...
    for _ in 0..minimal_iterations {
        let res = evaluate_patient_all_groups(
            &min_dose_case.patient,
            &min_dose_case.history,
            min_dose_case.execution_date,
        );
        black_box(res);
    }
    let elapsed_st = start.elapsed();
