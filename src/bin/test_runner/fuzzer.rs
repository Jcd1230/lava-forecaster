...
use lava_forecaster::{
    ...
    models::{
        Dose, DoseEvaluation, ExpectedResults,
...
pub fn generate_guided_case(
    ...
    for _ in 0..num_doses {
        ...
        let rust_group = rust_results.iter().find(|rg| rg.vaccine_group.to_uppercase() == target_group.to_uppercase());
        
        let mut possible_dates = Vec::new();
        if let Some(rg) = rust_group {
            if let Some(fc) = rg.forecasts.first() {
                if let SeriesStatus::NotComplete { earliest_date, recommended_date, overdue_date, .. } = &fc.status {
        ...
        let next_dose_date = if next_dose_date < dob { dob } else { next_dose_date };
    ...
    UnifiedTestCase {
        name: format!("fuzz_fail_{}_{}_{}", target_group.to_lowercase(), seed, case_index),
...
pub fn shrink_case(
    client: &Client,
    java_url: &str,
    mut tc: UnifiedTestCase,
) -> UnifiedTestCase {
    println!("Shrinking failing case: {} (originally {} doses)", tc.name, tc.history.len());
    let mut i = 0;
    while i < tc.history.len() {
        ...
        let rust_results = evaluate_patient_all_groups(&shrunked_tc.patient, &shrunked_tc.history, shrunked_tc.execution_date);
        let rust_group = rust_results.iter().find(|rg| rg.vaccine_group.to_uppercase() == shrunked_tc.group.to_uppercase());
